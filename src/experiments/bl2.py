"""
BL2: DCS Only
- Client selection via DCS at UAV level
...
async def main():
    ...
    for i in range(Config.NUM_CLIENTS):
        ...
        # Assign all clients to cluster 0 (no clustering in BL2)
    ...
    print(f"\n=== Starting BL2 (DCS Only) Simulation (Goal: Avg Test Acc {Config.TARGET_ACC}%) ===", flush=True)
    ...
    for r in range(Config.ROUNDS):
        ...
        for uav in uavs:
            ...
            # Aggregate cluster models (only cluster 0 for BL2)
        ...
        if sat.buffer:
            ...
            # For BL2, all UAVs send cluster 0 models
            cluster_0_models = []
            cluster_0_weights = []
            ...
            if cluster_0_models:
                ...
                print(f"  Aggregated {len(cluster_0_models)} UAV updates from DCS-selected clients.")
        ...
        if selected_total:
            log_cluster_assignments("bl2", r + 1, selected_total, sat.client_to_cluster)
